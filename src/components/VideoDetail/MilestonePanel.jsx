...
export default function MilestonePanel({ videoId, points, milestones }) {
  ...
  // local end-of-today; toISOString would shift by the tz offset
  const tonight = () => setDeadline(new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10) + 'T23:59')
  ...
  return <section className="detail-section milestone-section"><h3>MILESTONES</h3><div className="milestone-list">{milestones.map(item => {
    ...
    return <article className={item.hitAt ? 'is-hit' : ''} key={item.id}><div><b>{item.targetCount.toLocaleString()}</b><span>{item.deadlineTimestamp ? `Deadline ${new Date(item.deadlineTimestamp).toLocaleString()}` : 'No deadline'}</span></div>{probability && <div className="milestone-projection"><span>Projected at deadline</span><b className={probability.projectedAtDeadline >= item.targetCount ? '' : 'short'}>{probability.projectedAtDeadline.toLocaleString()} <i>{compact(probability.projectedRange.low)}–{compact(probability.projectedRange.high)}</i></b></div>}<div className="milestone-meta">{probability && <ProbabilityBadge {...probability} />}<span className="milestone-eta">{item.hitAt ? `Hit ${new Date(item.hitAt).toLocaleString()}` : eta ? `ETA ${new Date(eta.estimatedAt).toLocaleString()}` : 'No ETA'}</span><button className="plain" onClick={() => db.milestones.delete(item.id)} aria-label="Remove milestone">×</button></div></article>
  })}{!milestones.length && <p className="help">No targets defined.</p>}</div><details className="add-milestone"><summary>+ Add milestone</summary><form className="milestone-form" onSubmit={add}><input type="number" min="1" value={target} onChange={event => setTarget(event.target.value)} placeholder="Target views" /><input type="datetime-local" value={deadline} onChange={event => setDeadline(event.target.value)} /><button type="button" className="plain" onClick={tonight}>Tonight</button><button className="primary">Add target</button></form></details></section>
}
